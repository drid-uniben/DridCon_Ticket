...
- **Admin:** The superuser of the system. The Admin is responsible for managing agents (Front Desk) and attendees. They can add, invite, and manage all users. [implemented]
- **Front Desk (Agent):** These are the individuals responsible for verifying attendees at the conference venue. They will have access to a mobile application or web interface to scan QR codes. [implemented]
- **Attendee (User):** These are the conference attendees. They will receive a unique QR code via email after their registration is complete and payment is confirmed. [implemented]

## 3. User Flow

### 3.1. Attendee Flow

#### 3.1.1. Scenario 1: Manual Registration by Admin [implemented, human-testing-required]
...
#### 3.1.2. Scenario 2: Self-Registration by Attendee [implemented, human-testing-required]
...
#### 3.1.3. Scenario 3: Partial Registration by Admin & Completion by Attendee [implemented, human-testing-required]
...
1.  **Agent Login:** The Front Desk agent logs into the system using their provided credentials (received via email from the Admin). [implemented, human-testing-required]
...
3.  **QR Code Scan:** The agent uses a scanning device (e.g., a mobile web application after logging into the system) to scan the attendee's QR code. [implemented, human-testing-required]
4.  **System Validation & Feedback:** [implemented, human-testing-required]
    ...
    - **Already Scanned QR Code:**
      ...
      - The agent denies entry and escalates the issue if necessary.
5.  **Scan History:** The agent can view a history of users they have scanned, including their check-in status. [implemented, human-testing-required]

### 3.3. Admin Flow

1.  **Admin Login:** The Admin logs into the system with their credentials. [implemented, human-testing-required]
2.  **Agent Management:** [implemented, human-testing-required]
...
3.  **Attendee Management:** [implemented, human-testing-required]
...
4.  **Reporting and Tracking:**
  - **Monitor Check-ins:** The Admin can view real-time check-in status of all attendees, seeing which QR codes have been scanned and by which agent. [implemented, human-testing-required]
  - **Generate Reports:** The Admin can generate reports related to attendance, agent activity, and registration trends. [not-implemented, backlog]
...
- The system shall generate unique QR codes for each approved attendee. [implemented, human-testing-required]
- The system shall send email notifications to attendees (with QR codes) and agents (with credentials). [implemented, human-testing-required]
- The system shall ensure QR codes are invalidated after a single successful scan to prevent re-entry. [implemented, human-testing-required]
- The system shall record who checked in an attendee. [implemented, human-testing-required]
...
- The system shall enable attendees to present their QR code for scanning at entry.
 - The system shall allow attendees to self-register via a web form, providing personal details, ticket type, and proof of payment. [implemented, human-testing-required]
 - The system shall allow attendees to complete partial registrations initiated by an Admin. [implemented, human-testing-required]
 - The system shall display bank account details for payment during self-registration. [implemented, human-testing-required]
 - The system shall allow attendees to upload a receipt/proof of payment. [implemented, human-testing-required]
 - The system shall enable attendees to present their QR code for scanning at entry. [implemented, human-testing-required]
...
- The system shall maintain a history of scans performed by each agent.
 - The system shall allow agents to log in securely with unique credentials. [implemented, human-testing-required]
 - The system shall provide an interface for agents to scan attendee QR codes. [implemented, human-testing-required]
 - The system shall display immediate feedback upon QR code scan (valid, invalid, already scanned). [implemented, human-testing-required]
 - The system shall, for already scanned QR codes, display the name of the agent who performed the initial scan. [implemented, human-testing-required]
 - The system shall maintain a history of scans performed by each agent. [implemented, human-testing-required]
...
- The system shall enable the Admin to generate reports (attendance, agent activity, registration trends).
 - The system shall allow the Admin to securely log in. [implemented, human-testing-required]
 - The system shall allow the Admin to add new agents (Full Name, Email). [implemented, human-testing-required]
 - The system shall allow the Admin to manually add full attendee details (Full Name, Email, Phone Number, Ticket Type, Designation) and trigger QR code generation/email. [implemented, human-testing-required]
 - The system shall allow the Admin to initiate partial attendee registrations by sending an invitation link. [implemented, human-testing-required]
 - The system shall allow the Admin to review self-registered attendees, including their submitted details and proof of payment. [implemented, human-testing-required]
 - The system shall allow the Admin to approve or decline self-registrations. [implemented, human-testing-required]
 - The system shall provide the Admin with a comprehensive view of all attendees (registration status, payment status, check-in status). [implemented, human-testing-required]
 - The system shall enable the Admin to monitor real-time check-in status and identify who scanned which QR code. [implemented, human-testing-required]
 - The system shall enable the Admin to generate reports (attendance, agent activity, registration trends). [not-implemented, backlog]
...
- Admin dashboard operations (e.g., loading attendee lists, generating reports) shall be responsive, with typical actions completing within 5 seconds.
 - The QR code scanning and validation process shall provide a response within 2 seconds to ensure efficient attendee throughput. [not-implemented, needs-load-testing]
 - The system shall be capable of handling 20 concurrent agent logins and 100 concurrent QR code scans without degradation in performance. [not-implemented, needs-load-testing]
 - Admin dashboard operations (e.g., loading attendee lists, generating reports) shall be responsive, with typical actions completing within 5 seconds. [partially-implemented, performance-testing-required]
...
- QR codes shall be designed to be resistant to unauthorized reproduction or tampering.
 - All user (Admin, Agent, Attendee) data, especially personally identifiable information (PII) and payment details, shall be encrypted both in transit and at rest. [partially-implemented, infra-review-required]
 - User authentication for Admin and Agent roles shall be secure (e.g., using strong password policies, multi-factor authentication where appropriate). [implemented, human-testing-required]
 - The system shall protect against common web vulnerabilities (e.g., SQL injection, XSS). [partially-implemented, security-audit-required]
 - Access to administrative functions shall be restricted based on role-based access control. [implemented, human-testing-required]
 - QR codes shall be designed to be resistant to unauthorized reproduction or tampering. [partially-implemented, security-audit-required]
...
- The Admin dashboard shall provide a clear overview and efficient tools for managing agents and attendees.
 - The agent scanning interface shall be intuitive and easy to use, requiring minimal training. [implemented, human-testing-required]
 - Error messages and feedback shall be clear and actionable for both agents and attendees. [implemented, human-testing-required]
 - The attendee registration process (especially self-registration) shall be user-friendly and guided. [implemented, human-testing-required]
 - The Admin dashboard shall provide a clear overview and efficient tools for managing agents and attendees. [implemented, human-testing-required]
...
- The system shall have mechanisms for error logging and monitoring to facilitate quick issue resolution.
 - The system shall have an uptime of at least 99.5% during the conference period. [not-implemented, ops-required]
 - Data backups shall be performed regularly to prevent data loss. [not-implemented, ops-required]
 - The system shall have mechanisms for error logging and monitoring to facilitate quick issue resolution. [partially-implemented, ops-review-required]
...
- The system architecture shall be designed to scale to accommodate a growing number of attendees and agents for future conferences.
 - The system architecture shall be designed to scale to accommodate a growing number of attendees and agents for future conferences. [not-implemented, ops-required]
...
- The codebase shall be well-documented, modular, and follow established coding standards to facilitate future enhancements and maintenance.
 - The codebase shall be well-documented, modular, and follow established coding standards to facilitate future enhancements and maintenance. [partially-implemented, code-review-required]
...
- The system shall comply with relevant data privacy regulations (e.g., GDPR, CCPA) regarding the collection, storage, and processing of attendee data.
 - The system shall comply with relevant data privacy regulations (e.g., GDPR, CCPA) regarding the collection, storage, and processing of attendee data. [not-implemented, legal-review-required]
